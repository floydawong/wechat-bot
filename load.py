...
def load_config_to_bot(bot):
    ...
    bot.is_auto_busy_reply = config.is_auto_busy_reply
    ...
    # 发送机器人状态信息
    bot_status = bot_status if '文件助手' in bot_status else bot_status + bot_status_detail(
        bot)
...
def load_listen_friend(bot):
    ...
    if bot.is_listen_friend:
        ...
        if not bot.listen_friends:
            ...
            return '未在好友中找到备注为「{}」的监听对象！'.format(
                str(config.listen_friend_names))

        bot.listen_friend_groups = bot.groups().search(
            config.listen_friend_groups)
...
def load_listen_sharing_groups(bot):
    ...
    if bot.is_listen_sharing:
        bot.listen_sharing_groups = bot.groups().search(
            config.listen_sharing_groups)
...
def bot_status_detail(bot):
    ...
    bot_config_status += '\n机器人管理员：{0}（{1}）'.format(bot.master.remark_name,
                                                    bot.master.nick_name)
    if bot.is_forward_mode:
        bot_config_status += '\n转发模式已开启，您发送给我的任何信息都将被转发至:{}，您可发送命令：关闭转发模式 来关闭转发模式。'.format(
            str(bot.forward_groups))
    bot_config_status += '\n好友自动回复：{}'.format(('是' if bot.is_friend_auto_reply
                                               else '否'))

    bot_config_status += '\n群聊回复：{}'.format(('是'
                                             if bot.is_group_reply else '否'))
    if bot.is_group_reply:
        bot_config_status += '，是否需要@才回复：{}'.format('是' if bot.is_group_at_reply
                                                   else '否')
    ...
    bot_config_status += '\n是否开启监听模式：{}'.format('是' if bot.is_listen_friend
                                                else '否')
    if bot.is_listen_friend:
    ...
    bot_config_status += '\n是否开启监控模式：{}'.format('是' if bot.is_listen_sharing
                                                else '否')
    if bot.is_listen_sharing:
...
def search_friend(bot, name):
    ...
    for friend in bot.friends():
        if getattr(friend, 'remark_name', None) == name:
            return friend
        elif not nick_name_friend and getattr(friend, 'nick_name',
                                              None) == name:
